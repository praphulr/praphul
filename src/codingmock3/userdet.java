...
class UserBO1{    
    ...
    public void display (User1 [] users) {        
        System.out.printf("%5s %20s %10s %10s  %10s", "ID", "NAME", "MOBILE NUMBER","USERNAME", "PASSWORD");
        for(int i=0;i<users.length;i++)
        {
            System.out.println();
            System.out.printf( "%5s %20s %10d %10s %10s", (i+1),users[i].getName(),users[i].getMobileNumber(),users[i].getUsername(), users[i].getPassword());
        }
        System.out.println();
    }
